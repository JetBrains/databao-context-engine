...
def test_build_enriches_before_export_and_index(stub_sources, stub_prepare, mock_build_service, project_layout):
    ...
    mock_build_service.enrich_built_context.assert_called_once_with(built_context)
    mock_build_service.index_built_context.assert_called_once()
    index_call = mock_build_service.index_built_context.call_args
...
def test_run_indexing_returns_ok_result(mock_build_service, project_layout):
    ...
    assert len(results) == 1
    assert results[0].datasource_id == ctx.datasource_id
    assert results[0].status == DatasourceStatus.OK
...
def test_run_indexing_skips_when_plugin_missing(mock_build_service, project_layout):
    context = _context("files/missing.md", context=to_yaml_string({"type": "files/md"}))
    mock_build_service.index_datasource_context.side_effect = NoPluginFoundForDatasource(
...
def test_build_skips_disabled_config_source(stub_sources, stub_prepare, mock_build_service, project_layout):
    datasource_id = _datasource_id("configs/my_source.yaml")
    stub_sources([datasource_id])
