...
@pytest.fixture()
def dummy_plugins_by_type():
    return load_dummy_plugins()
...
def test_databao_context_project_manager__create_datasource_config__fails_invalid_config_content(
    project_path, dummy_plugins_by_type
):
    project_manager = DatabaoContextProjectManager(
        project_dir=project_path, plugin_loader=DatabaoContextPluginLoader(plugins_by_type=dummy_plugins_by_type)
    )
...
def test_databao_context_project_manager__create_datasource_config__valid_config_content_dict(
    project_path, dummy_plugins_by_type
):
    project_manager = DatabaoContextProjectManager(
        project_dir=project_path, plugin_loader=DatabaoContextPluginLoader(plugins_by_type=dummy_plugins_by_type)
