...
def test_embed_embedding_field():
    ...
    svc = OllamaService(OllamaConfig(host="host", port=11434, timeout=12.0), session=session)

    vec = svc.embed(model="nomic-embed-text", text="hello")
...
def test_embed_alt_schema_under_data_embedding():
    ...
    svc = OllamaService(OllamaConfig(host="x"), session=session)

    vec = svc.embed(model="m", text="t")
    assert vec == [0.1, 0.2]
...
def test_pull_model_waits_until_completion():
    ...
    svc = OllamaService(OllamaConfig(host="host", port=11434), session=sess)

    svc.pull_model(model="nomic-embed-text", timeout=5.0)
...
def test_pull_model_http_error_raises_with_body_snippet():
    ...
    svc = OllamaService(OllamaConfig(host="x"), session=sess)

    with pytest.raises(requests.HTTPError) as ei:
        svc.pull_model(model="m", timeout=1.0)
    assert "body:" in str(ei.value)
...
def test_pull_model_timeout_raises_timeouterror():
    ...
    with pytest.raises(TimeoutError):
        svc.pull_model(model="m", timeout=0.01)


def test_is_healthy_true_on_2xx():
    session = _StubSession()
    session.set_next_get(_StubResponse(status=200))
    svc = OllamaService(OllamaConfig(host="h", port=11434), session=session)

    assert svc.is_healthy(timeout=0.1) is True
    call = session.calls[0]
...
def test_is_healthy_false_on_non_2xx():
    ...
    svc = OllamaService(OllamaConfig(host="h"), session=session)
...
def test_wait_until_healthy_becomes_true(monkeypatch):
    ...
    monkeypatch.setattr(svc, "is_healthy", lambda timeout=0.5: next(states))
    monkeypatch.setattr("time.sleep", lambda _: None)

    assert svc.wait_until_healthy(timeout=5.0, poll_interval=0.01) is True
...
def test_wait_until_healthy_times_out(monkeypatch):
    ...
    monkeypatch.setattr(svc, "is_healthy", lambda timeout=0.5: False)
    monkeypatch.setattr("time.sleep", lambda _: None)

    assert svc.wait_until_healthy(timeout=0.05, poll_interval=0.01) is False
